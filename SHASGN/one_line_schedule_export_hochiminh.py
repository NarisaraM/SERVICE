"""
one_line_schedule_export_hochiminh.py
======================================
...
ค่าเริ่มต้นของสคริปต์นี้ถูกตั้งไว้ตามคำขอ:
    ...
    Destination (POD) : HO CHI MINH, VIETNAM         (VNSGN)
    ช่วงวันที่ (ETD)   : 2026-09-01 ถึง 2026-11-30 (กันยายน - พฤศจิกายน 2026)

รหัสท่าเรือ CNSHA / VNSGN และชื่อท่าเรือด้านบน ตรวจสอบมาจาก URL จริงที่หน้าเว็บ
ONE สร้างขึ้นเวลากดค้นหา Shanghai -> Ho Chi Minh บนหน้าเว็บ ไม่ได้เดาเอง
...
2) รันสคริปต์ (ใช้ค่าเริ่มต้น Shanghai -> Ho Chi Minh, ก.ย.-พ.ย. 2026):

    python one_line_schedule_export_hochiminh.py

   หรือกำหนดพารามิเตอร์เอง เช่นเปลี่ยนเส้นทาง/ช่วงวันที่:

    python one_line_schedule_export_hochiminh.py \
        --origin-code CNSHA --origin-name "SHANGHAI, SHANGHAI, CHINA" \
        --dest-code VNSGN   --dest-name "HO CHI MINH, VIETNAM" \
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--dest-code", default="VNSGN", help="รหัสท่าเรือปลายทาง (UN/LOCODE)")
    parser.add_argument("--dest-name", default="HO CHI MINH, VIETNAM", help="ชื่อท่าเรือปลายทางตามที่เว็บ ONE ใช้")
