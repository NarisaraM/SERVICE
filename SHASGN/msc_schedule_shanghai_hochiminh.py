#!/usr/bin/env python3
"""
ดึงตารางเรือ MSC (Point-to-Point) จาก Shanghai -> Ho Chi Minh City
...
API ที่หน้าเว็บ https://www.msc.com/en/search-a-schedule เรียกใช้จริง:
    ...
    body: {"FromDate": "YYYY-MM-DD", "fromPortId": 444, "toPortId": 52,
           "language": "en", "dataSourceId": "{E9CCBD25-...}"}
    header ที่จำเป็น: X-Requested-With: XMLHttpRequest
    (fromPortId 444 = SHANGHAI, toPortId 52 = HO CHI MINH CITY, VNSGN)
    เส้นทางนี้มีทั้ง Direct (SAOLA) และ Transhipment (BRITANNIA) สคริปต์เก็บไว้ทั้งสองแบบ
...
รัน:      python msc_schedule_shanghai_hochiminh.py
          python msc_schedule_shanghai_hochiminh.py --year 2026 --start-month 9 --end-month 11
"""
import argparse
import calendar
import sys
...
TO_PORT_ID = 52      # HO CHI MINH CITY (VNSGN)
...
def main():
    ap = argparse.ArgumentParser(description="MSC schedule Shanghai -> Ho Chi Minh City")
    ap.add_argument("--year", type=int, default=date.today().year)
    ap.add_argument("--start-month", type=int, default=9)
    ap.add_argument("--end-month", type=int, default=11)
    ap.add_argument("--out", default="MSC_Shanghai_HoChiMinh")
    args = ap.parse_args()
