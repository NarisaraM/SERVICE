...
ค่าเริ่มต้นของสคริปต์นี้ถูกตั้งไว้ตามคำขอ:
    ...
    Destination (POD) : LAEM CHABANG, THAILAND       (THLCH)
...
2) รันสคริปต์ (ใช้ค่าเริ่มต้น Shanghai -> Laem Chabang, ก.ย.-พ.ย. 2026):

    python one_line_schedule_export.py

   หรือกำหนดพารามิเตอร์เอง เช่นเปลี่ยนเส้นทาง/ช่วงวันที่:

    python one_line_schedule_export.py \
        --origin-code CNSHA --origin-name "SHANGHAI, SHANGHAI, CHINA" \
        --dest-code THLCH   --dest-name "LAEM CHABANG, THAILAND" \
        --start 2026-09-01 --end 2026-11-30 \
        --output ONE_Shanghai_LaemChabang_Sep-Nov2026.xlsx
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--dest-code", default="THLCH", help="รหัสท่าเรือปลายทาง (UN/LOCODE)")
    parser.add_argument("--dest-name", default="LAEM CHABANG, THAILAND", help="ชื่อท่าเรือปลายทางตามที่เว็บ ONE ใช้")
