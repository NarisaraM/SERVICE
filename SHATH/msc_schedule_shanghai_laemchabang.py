#!/usr/bin/env python3
"""
ดึงตารางเรือ MSC (Point-to-Point) จาก Shanghai -> Laem Chabang
...
API ที่หน้าเว็บ https://www.msc.com/en/search-a-schedule เรียกใช้จริง:
    ...
    body: {"FromDate": "YYYY-MM-DD", "fromPortId": 444, "toPortId": 36,
           "language": "en", "dataSourceId": "{E9CCBD25-...}"}
    header ที่จำเป็น: X-Requested-With: XMLHttpRequest
    (fromPortId 444 = SHANGHAI, toPortId 36 = LAEM CHABANG)
...
รัน:      python msc_schedule_shanghai_laemchabang.py
          python msc_schedule_shanghai_laemchabang.py --year 2026 --start-month 9 --end-month 11
"""
import argparse
import calendar
import sys
...
TO_PORT_ID = 36      # LAEM CHABANG (THLCH)
...
def collect(year: int, m_start: int, m_end: int) -> pd.DataFrame:
    ...
    while cursor <= win_end:
        ...
        new_last = None
        for row in rows:
            key = (row["Vessel"], row["Voyage"], row["ETD"])
...
def main():
    ap = argparse.ArgumentParser(description="MSC schedule Shanghai -> Laem Chabang")
    ap.add_argument("--year", type=int, default=date.today().year)
    ap.add_argument("--start-month", type=int, default=9)
    ap.add_argument("--end-month", type=int, default=11)
    ap.add_argument("--out", default="MSC_Shanghai_LaemChabang")
    args = ap.parse_args()
    ...
    print(df[["Vessel", "Voyage", "ETD", "ETA", "TransitTime"]].to_string(index=False))
