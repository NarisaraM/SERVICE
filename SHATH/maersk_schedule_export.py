...
Pre-configured for: Shanghai, China (CNSGH)  ->  Laem Chabang, Thailand (THLZP)
...
* The page/URL parameters below (from, to, fromRkstCode, toRkstCode, ...)
  were captured from the live "Shanghai -> Laem Chabang" schedule search on
...
# Location codes as used by maersk.com for this lane (Shanghai -> Laem Chabang)
FROM_LOCODE = "2IW9P6J7XAW72"   # Shanghai, China
TO_LOCODE = "0NTCCB4ENSFX8"     # Laem Chabang, Thailand
...
OUTPUT_FILE = "maersk_shanghai_to_laemchabang_sep_nov_2026.xlsx"
...
def parse_schedule(html: str):
    ...
    while i < n:
        if (
            ...
            and lines[i + 8].startswith("Transit Time:")
        ):
            dep_date, dep_port = lines[i + 1], lines[i + 2]
            arr_date, arr_port = lines[i + 4], lines[i + 5]
            vessel_voyage = lines[i + 7]
            transit = lines[i + 8].replace("Transit Time:", "").strip()

            m = re.match(r"^(.*?)(\d+\w)$", vessel_voyage)
            vessel = m.group(1).strip() if m else vessel_voyage
            voyage = m.group(2) if m else ""
            ...
            j = i + 9
            while j < n and lines[j] not in ("Departure",) and lines[j] != "Show route details":
                if lines[j] in DEADLINE_LABELS and j + 1 < n:
                    row[lines[j]] = lines[j + 1]
                    j += 2
                    continue
                j += 1
...
def fetch_range(start: date, end: date):
    ...
    with sync_playwright() as p:
        ...
        for window_start in date_windows(start, end):
            ...
            for row in rows:
                key = (row["Departure Date"], row["Vessel"], row["Voyage"])
                if key not in seen:
                    seen.add(key)
                    all_rows.append(row)
