#!/usr/bin/env python3
"""
hmm_schedule_scraper_shanghai_manzanillo.py
=============================================

Scrapes the HMM (Hyundai Merchant Marine) e-Service "Point to Point" sailing
schedule page for SHANGHAI, CHINA -> MANZANILLO, MEXICO and saves the results
to an Excel (.xlsx) and CSV file.
...
Default lane / period (edit the CONFIG section below to change):
    ...
    Destination : MANZANILLO, MEXICO  (MXZLO)
    Period      : 2026-09-01 -> 2026-11-30

NOTE ON "MANZANILLO": HMM's destination list has two ports with this name -
    MANZANILLO, MEXICO [MXZLO]  <- used by default below
    MANZANILLO, PANAMA [PAMIT]
If you actually meant Panama, run with:
    python hmm_schedule_scraper_shanghai_manzanillo.py --destination MANZANILLO --destination-code PAMIT
...
USAGE
-----
    python hmm_schedule_scraper_shanghai_manzanillo.py
    python hmm_schedule_scraper_shanghai_manzanillo.py --start 2026-09-01 --end 2026-11-30 --headed
...
DEFAULT_DESTINATION = "MANZANILLO"   # text typed into the Destination autocomplete box
DEFAULT_DESTINATION_CODE = "MXZLO"   # MANZANILLO, MEXICO (use PAMIT for MANZANILLO, PANAMA)
...
#   "WK 45 Origin 2026-11-07 SHANGHAI,CHINA 20 days Direct Destination
#    2026-11-27 MANZANILLO, MEXICO Main Vessel HMM XXXX (001W)
#    Route ABC Operator HMM 1st Vessel HMM XXXX (001W)
...
def select_autocomplete(page, input_id: str, query: str, expect_code: str | None = None):
    ...
    click the matching suggestion. When more than one port shares a name
    (e.g. MANZANILLO, MEXICO vs MANZANILLO, PANAMA) expect_code disambiguates."""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Scrape HMM sailing schedule (Point to Point) for Shanghai -> Manzanillo."
    )
    ...
    parser.add_argument("--destination", default=DEFAULT_DESTINATION, help="Destination search text, e.g. MANZANILLO")
    parser.add_argument(
        "--destination-code",
        default=DEFAULT_DESTINATION_CODE,
        help="Destination UN/LOCODE: MXZLO = Manzanillo, Mexico (default); PAMIT = Manzanillo, Panama",
    )
